// UI Module for Doc Tailor Extension
// Handles FAB, flyout, and menu creation
...
function createFlyout() {
  ...
  // Only show "Open Doc Tailor" menu item on job view pages, not on search pages
  if (!window.DocTailorUtils.isJobSearchPage()) {
    // First menu item - Open Doc Tailor
    openMenuItem = document.createElement('button');
    ...
    // On job search pages, add "Open All Jobs" menu item
    const openAllJobsMenuItem = document.createElement('button');
    openAllJobsMenuItem.style.cssText = `
      width: 100%;
      padding: 12px 16px;
      border: none;
      background: none;
    ...
    openAllJobsMenuItem.addEventListener('mouseenter', () => {
      openAllJobsMenuItem.style.backgroundColor = 'oklch(0.95 0.01 195)';
    });

    openAllJobsMenuItem.addEventListener('mouseleave', () => {
      openAllJobsMenuItem.style.backgroundColor = 'transparent';
    });

    // Multiple windows icon for "Open All Jobs"
    const multipleIcon = document.createElementNS(svgNS, 'svg');
    multipleIcon.setAttribute('viewBox', '0 0 24 24');
    multipleIcon.setAttribute('width', '16');
    multipleIcon.setAttribute('height', '16');
    multipleIcon.setAttribute('aria-hidden', 'true');
    multipleIcon.style.fill = 'oklch(0.2 0.01 195)';

    const multiplePath = document.createElementNS(svgNS, 'path');
    multiplePath.setAttribute('d', 'M3 3h8v2H5v12h12v-6h2v8H3V3zm18 0h-8v2h6v12h-6v2h8V3z');
    multipleIcon.appendChild(multiplePath);

    const openAllJobsLabel = document.createElement('span');
    openAllJobsLabel.textContent = 'Open All Jobs';

    openAllJobsMenuItem.appendChild(multipleIcon);
    openAllJobsMenuItem.appendChild(openAllJobsLabel);
    flyout.appendChild(openAllJobsMenuItem);

    // Add click handler for "Open All Jobs"
    openAllJobsMenuItem.addEventListener('click', () => {
      if (window.DocTailorJobHandlers && window.DocTailorJobHandlers.openAllJobLinks) {
        window.DocTailorJobHandlers.openAllJobLinks();
      }
      hideFlyout();
    });
  ...
  if (openMenuItem || window.DocTailorUtils.isJobSearchPage()) {
...
function updateFABPosition(position) {
  ...
  if (pos) {
    ...
    try {
      window.DocTailorUtils.getStore().set({ fabPosition: position }, () => {});
    } catch (e) {
...
async function setupFAB() {
  ...
  // Load saved position
  const savedPosition = await window.DocTailorUtils.loadSavedPosition();
  ...
  // FAB click to open Doc Tailor directly (disabled on job search pages)
